...
// ============================================================================
class gWeapon extends gWeaponBase
    Abstract
    HideDropDown
    CacheExempt
    Config;
...
var() config bool       bAccurateCrosshair;
...
simulated event Destroyed()
{
    ...
    {
        if( WarrantyMode != 0 && gPawn(Instigator) != None && gPawn(Instigator).ShopInfo != None )
            gPawn(Instigator).ShopInfo.RefundWarranty(self);
    }
...
function GiveToEx(Pawn Other, optional Pickup Pickup, optional bool bGiveEmpty)
{
    ...
    if( gWeaponPickup(Pickup) != None )
    {
        if( gWeapon(W) != None )
        {
        	// Red warranty overrides all
        	if( gWeaponPickup(Pickup).WarrantyMode == 1 )
        	{
	        	gWeapon(W).WarrantyMode = gWeaponPickup(Pickup).WarrantyMode;
        	}
	            
	        // Green warranty overrides no warranty
        	if( gWeapon(W).WarrantyMode == 0 && gWeaponPickup(Pickup).bBloody && gWeaponPickup(Pickup).KilledController != Instigator.Controller )
        	{
